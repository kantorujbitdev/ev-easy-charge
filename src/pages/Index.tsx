    ...
    <div className="space-y-6">
      ...
      {isCharging ? (
        <Card className="border-primary shadow-md">
          <CardHeader className="pb-2">
            ...
            <CardDescription>
              {chargers.find(c => c.id === currentSession.stationId)?.name || 'Station'}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col items-center p-3 bg-muted/50 rounded-lg">
                ...
                <span className="text-xl font-semibold">{currentSession.kWh.toFixed(2)} kWh</span>
              </div>
              <div className="flex flex-col items-center p-3 bg-muted/50 rounded-lg">
                <Clock className="h-5 w-5 text-primary mb-1" />
                <span className="text-sm text-muted-foreground">Duration</span>
                <span className="text-xl font-semibold">{formatDuration(currentSession.elapsedTime)}</span>
              </div>
            </div>
        ...
        <Card className="border-muted shadow-sm">
          <CardHeader className="pb-2">
            ...
            <CardDescription>Find an available charging station</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{availableChargers} stations ready</p>
                </div>
              </div>
              <Button 
                size="sm" 
      ...
      <section className="grid gap-4 grid-cols-1 md:grid-cols-2">
        <Card>
          ...
          <CardContent>
            <p className="text-muted-foreground mb-4">View your recent charging sessions</p>
            <Button 
              variant="outline" 
              className="w-full"
